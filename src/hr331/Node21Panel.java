...
public class Node21Panel extends JPanel implements ActionListener,ItemListener,ListSelectionListener{
	...
	//新部门
		...
		newDeptCom = new JComboBox<Object>(newDept);//将数据库的数据加到JComboBox部件
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource() == clear){
			...
			if(nameText.getText().trim().length()!=0) {
				...
				oriDeptId=pbean.getDeptId(PersonId);//得到旧部门编号
				oriDept = pbean.getDept(oriDeptId);//得到旧部门名字
				
				pbean.updateDept(PersonId, deptId);//更新部门信息
				
				nowDept = pbean.getDept(deptId);//得到新部门名字
				...
				hbean.add(jourNoStr, "人员调动",oriDept,nowDept, regDate, chgTime,PersonId);
				table.revalidate();
				JOptionPane.showMessageDialog(null,"人员调动成功!");
				HrMain.reDraw(this.getClass().getName());//解决刷新问题
			}
			else
				JOptionPane.showMessageDialog(null, "请先选择您要调动的员工！");
